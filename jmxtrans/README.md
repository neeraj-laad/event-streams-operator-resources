...
JmxTrans is a Java application, so a container image that includes a Java runtime and the JmxTrans tool is needed to deploy it on Kubernetes environments. JmxTrans does not support Java 17 yet, so Java 11 is required. 

Event Streams 11.1.6 (and earlier) included a container image for running the JmxTrans tool. For example, `cp.icr.io/cp/ibm-eventstreams-jmxtrans@sha256:c43965013d22d3227cf7c1bf1211d59b0595ce6d535ab3fab8e7c80694417f76`

> Note: This image is no longer maintained and does not receive security updates. We recommend that you build a custom image.
